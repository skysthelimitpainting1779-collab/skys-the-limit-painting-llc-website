...
export const DEFAULT_SETTINGS: CompanySettings = {
  ...
  meta_title_default:
    "Sky's the Limit Painting LLC | Premium Painting & Striping",
  meta_desc_default:
    'Professional painting contractor serving the Twin Cities Metro. Quality prep, clean lines, and durable coatings.',
};
...
export async function getCompanySettings(): Promise<CompanySettings> {
  try {
    ...
    if (error) {
      console.warn(
        `Settings DB query failed (code: ${error.code}): ${error.message}. Using defaults.`
      );
