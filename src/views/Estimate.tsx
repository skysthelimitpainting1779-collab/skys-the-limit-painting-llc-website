...
export default function EstimatePage() {
  ...
  useEffect(() => {
    try {
      ...
      console.warn(
        'Failed to save estimate progress (storage may be full):',
        err
      );
    }
  ...
  const calculateCosts = () => {
    ...
    const wallPrepValue =
      trimPrep.prepLevel === 'premium' ? wallBase * 0.35 : 0;
    const wallCost = wallBase + wallPrepValue;

    // Add doors ($150 each) and windows ($100 each)
    const openingsCost =
      trimPrep.doorsCount * 150 + trimPrep.windowsCount * 100;

    // Trim painting cost: linear feet * $4.00 per foot
    const trimCost = trimPrep.trimLength * 4.0;
    ...
    // Return ranges
    const lowRange = Math.round(totalEstimate * 0.9);
  ...
  const handleFinalSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (botHoneypot) {
      setStatus('sent');
      setMessage(
        'Your planning range was received. Sky’s the Limit can follow up to confirm surface condition and final scope.'
      );
      return;
    }
...
Calculated Range: $${calculationResult.low} - $${calculationResult.high}${projectAddress ? `\nAddress: ${projectAddress}` : ''}`,
    ...
    if (typeof window !== 'undefined' && !navigator.onLine) {
      ...
      setStatus('fallback');
      setMessage(
        'Offline Mode: Your estimate calculation is saved locally. Anthony will review it once your connection returns.'
      );
      trackEvent('lead_offline_queued', {
        source: 'Estimate Calculator',
    ...
    try {
      ...
      if (response.ok && result?.ok === true) {
        ...
        setStatus('fallback');
        setMessage(
          'Lead service is currently starting. Tap the button below to draft your request directly in your email client.'
        );
      }
    } catch {
      setStatus('fallback');
        ...
        <div className="relative z-10 mx-auto max-w-7xl">
          <div className="text-center mb-12">
            ...
            <p className="mt-4 text-base text-[#c9c1b4] max-w-[65ch] mx-auto">
              Tweak dimensions, trim level, and cabinets below. See your
              planning range update in real-time. Lock in your rate to request a
              confirmed owner-led site walkthrough.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="col-span-12 lg:col-span-7 space-y-8">
                    ...
                    {[
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setDimensions({ ...dimensions, roomType: type })
                          }
              ...
              <div className="border border-white/10 bg-[#0B0B0D]/95 p-6 backdrop-blur-md space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <RangeSlider
                    ...
                    max={50}
                    suffix="FT"
                    onChange={(v) => setDimensionValue('width', v, 5, 50)}
                  />
                  ...
                  <RangeSlider
                    ...
                    max={50}
                    suffix="FT"
                    onChange={(v) => setDimensionValue('length', v, 5, 50)}
                  />
                  ...
                  <RangeSlider
                    ...
                    suffix="FT"
                    onChange={(v) =>
                      setDimensionValue('ceilingHeight', v, 7, 20)
                    }
                  />
                </div>
              </div>
              ...
              <div className="border border-white/10 bg-[#0B0B0D]/95 p-6 backdrop-blur-md space-y-6">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <button
                        type="button"
                        onClick={() =>
                          setTrimPrep({ ...trimPrep, prepLevel: 'standard' })
                        }
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-400 mt-2">
                          Light sanding, minor caulk, 1 coat primer &amp;
                          topcoat. Great for minor refreshes.
                        </p>
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                          setTrimPrep({ ...trimPrep, prepLevel: 'premium' })
                        }
                        className={`p-4 border text-left transition-all ${
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-400 mt-2">
                          Elite multi-stage sanding, deep caulking, wood
                          stabilization, 2 premium coats. Luxury finish.
                        </p>
                      </button>
                    </div>
                  </div>
                  ...
                  <RangeSlider
                    ...
                    min={0}
                    max={10}
                    onChange={(v) => setTrimValue('doorsCount', v, 0, 10)}
                  />
                  ...
                  <RangeSlider
                    ...
                    min={0}
                    max={10}
                    onChange={(v) => setTrimValue('windowsCount', v, 0, 10)}
                  />

                  {/* Est. Trim Slider */}
                  <RangeSlider
                    id="trim-length"
                    label="Est. Trim (linear ft)"
                    value={trimPrep.trimLength}
                    min={0}
                    max={200}
                    step={5}
                    suffix="FT"
                    onChange={(v) => setTrimValue('trimLength', v, 0, 200)}
                  />
                </div>
              </div>
              ...
              <div className="border border-white/10 bg-[#0B0B0D]/95 p-6 backdrop-blur-md space-y-6">
                <div className="flex items-center gap-3 border-b border-white/5 pb-4">
                  ...
                  <h2 className="text-base font-bold text-white">
                    Cabinets Painting Add-On
                  </h2>
                </div>

                <p className="text-sm text-gray-400">
                  Cabinet transformations can modernize a kitchen without full
                  replacement. Add openings below to include this scope.
                </p>
                ...
                <div className="space-y-6">
                  ...
                  <RangeSlider
                    ...
                    min={0}
                    max={60}
                    onChange={(v) => setCabinetValue('cabinetDoors', v, 0, 60)}
                  />
                  ...
                  <RangeSlider
                    ...
                    max={40}
                    onChange={(v) =>
                      setCabinetValue('cabinetDrawers', v, 0, 40)
                    }
                  />
                </div>
              </div>
            ...
            <div className="col-span-12 lg:col-span-5 lg:sticky lg:top-24 space-y-6">
              <div className="border border-white/10 bg-[#0B0B0D]/95 p-6 backdrop-blur-md">
                <p className="text-xs font-semibold text-center text-[#c9c1b4] uppercase tracking-widest">
                  Estimated Interior Price Range
                </p>
                <p className="mt-3 font-display text-5xl font-black text-white">
                  ${calculationResult.low.toLocaleString()} – $
                  {calculationResult.high.toLocaleString()}
                </p>
                <p className="mt-2 text-sm text-gray-400">
                  Approx. {calculationResult.wallArea} sq ft of wall surface
                  area.
                </p>
                ...
                <div className="mt-6 border-t border-white/10 pt-5 space-y-3 text-sm">
                  ...
                  <div className="flex justify-between text-gray-400 border-b border-white/5 pb-2">
                    <span>
                      Wall prep, priming &amp; coatings ({dimensions.roomType})
                    </span>
                    <span className="font-bold text-white">
                      ${calculationResult.wallCost.toLocaleString()}
                    </span>
                  </div>
                  ...
                  {calculationResult.openingsCost > 0 && (
                    <div className="flex justify-between text-gray-400 border-b border-white/5 pb-2">
                      <span>
                        Doors ({trimPrep.doorsCount}) &amp; Windows (
                        {trimPrep.windowsCount})
                      </span>
                      <span className="font-bold text-white">
                        ${calculationResult.openingsCost.toLocaleString()}
                      </span>
                    </div>
                  )}
                  ...
                  {calculationResult.trimCost > 0 && (
                    <div className="flex justify-between text-gray-400 border-b border-white/5 pb-2">
                      ...
                      <span className="font-bold text-white">
                        ${calculationResult.trimCost.toLocaleString()}
                      </span>
                    </div>
                  )}
                  ...
                  {calculationResult.cabinetCost > 0 && (
                    <div className="flex justify-between text-gray-400 border-b border-white/5 pb-2">
                      <span>
                        Cabinets (
                        {cabinets.cabinetDoors + cabinets.cabinetDrawers}{' '}
                        openings)
                      </span>
                      <span className="font-bold text-white">
                        ${calculationResult.cabinetCost.toLocaleString()}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between font-bold text-white">
                    ...
                    <span>
                      ${calculationResult.totalEstimate.toLocaleString()}
                    </span>
                  </div>
                  <div className="mt-5 pt-4 border-t border-white/10 text-sm leading-relaxed text-[#c9c1b4]">
                    ...
                    Direct owner-operator structure. This range covers field
                    labor and premium coatings (Benjamin Moore or
                    Sherwin-Williams). No franchise markups, middleman fees, or
                    commissions.
                  </div>
                </div>
              </div>
                  ...
                  <form onSubmit={handleFinalSubmit} className="space-y-6">
                    <div>
                      ...
                      <p className="text-sm text-gray-300">
                        Send the calculated range with your details to request
                        an on-site walkthrough and secure your rate.
                      </p>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        <label
                          htmlFor="client-name"
                          className="block text-xs font-semibold text-[#c9c1b4] mb-1"
                        >
                          Full Name
                        </label>
                        <input
                    ...
                    <p
                      className="flex items-start gap-2 text-sm leading-relaxed text-[#b9b2a6]"
                      aria-live="polite"
                    >
                      <ShieldCheck
                        size={14}
                      ...
                      />
                      {message ||
                        'Your planning range routes with the room details. Final estimates are confirmed after surface condition and scope are reviewed.'}
                    </p>
                  </form>
                ) : (
                  <div className="py-6">
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-gray-400">
                      Thanks, {name}. Your planning range of $
                      {calculationResult.low.toLocaleString()} – $
                      {calculationResult.high.toLocaleString()} has been sent.
                      Sky's the Limit will follow up to confirm the final scope.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 border-t border-white/10 pt-6 mt-2">
                ...
                <div className="lg:col-span-5 flex flex-col justify-start space-y-4 border-t lg:border-t-0 lg:border-l border-white/10 pt-6 lg:pt-0 lg:pl-6">
                  <h4 className="text-xs font-semibold text-[#c9c1b4] uppercase tracking-widest">
                    Local Service Proof
                  </h4>
