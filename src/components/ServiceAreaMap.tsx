      ...
      <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-10 lg:grid-cols-12 lg:items-center">
        <motion.div {...mapMotion} className="lg:col-span-5">
          ...
          <h2
            ...
            className="mt-4 text-3xl font-black leading-tight text-white md:text-5xl"
          >
            Twin Cities coverage with live interactive mapping.
          </h2>
          <p className="mt-5 text-base leading-relaxed text-[#d7e3df] md:text-lg">
            A real-time, responsive Leaflet coverage map for homeowners and
            commercial property managers verifying whether our painting teams
            reach their local site.
          </p>
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2">
            {proofItems.map((item) => (
              <div
                key={item}
                className="flex items-start gap-3 border-l border-white/35 bg-white/[0.04] px-4 py-3 text-sm font-semibold text-[#eef7f4]"
              >
                <MapPin
                  aria-hidden="true"
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Link
              href="/contact"
              onClick={() =>
                trackEvent('service_area_map_cta_click', { action: 'estimate' })
              }
            ...
            <a
              ...
              rel="noreferrer"
              onClick={() =>
                trackEvent('service_area_map_cta_click', {
                  action: 'directions',
                })
              }
        ...
        <motion.figure {...mapMotion} className="lg:col-span-7">
          ...
          >
            <span id="service-area-visual-title" className="sr-only">
              Map of Sky's the Limit Painting service coverage across the Twin
              Cities metro.
            </span>
            <span id="service-area-visual-description" className="sr-only">
              Map markers include Minneapolis, St. Paul, Inver Grove Heights,
              South St. Paul, Eagan, Woodbury, and broad Twin Cities coverage.
            </span>
          ...
          <figcaption className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {serviceAreaPins.map((pin) => (
              <Link
                ...
                onClick={() =>
                  trackEvent('service_area_map_list_click', { area: pin.slug })
                }
              ...
              >
                <span>
                  {pin.name}
                  <span className="block text-xs font-semibold text-[#9fb0aa]">
                    {pin.region}
                  </span>
                </span>
