        ...
        <div className="relative z-10 mx-auto grid w-full max-w-7xl grid-cols-1 gap-12 lg:grid-cols-12 lg:items-end">
          <FadeIn className="w-full overflow-hidden lg:col-span-7">
            <div className="mb-8 flex items-center gap-4 text-white">
              <span className="font-display text-6xl font-black leading-none opacity-70">
                {market.number}
              </span>
            ...
            <p className="mt-8 max-w-[calc(100vw-2rem)] text-lg leading-relaxed text-gray-300 sm:max-w-3xl md:text-2xl">
              {market.headline}
            </p>
            <div className="mt-8 grid max-w-[calc(100vw-2rem)] grid-cols-1 gap-3 sm:max-w-3xl sm:grid-cols-3">
              {market.proof.map((item) => (
                <div key={item} className="border-t border-white/10 pt-4">
                  <p className="break-words text-xs font-black leading-relaxed ] text-gray-300 sm:">
                    {item}
                  </p>
                </div>
              ))}
          ...
          >
            <div className="grid w-full overflow-hidden border border-white/15 bg-[#11100d]/90 backdrop-blur">
              ...
              <div className="p-6">
                <p className="max-w-full text-lg font-semibold leading-relaxed text-white sm:max-w-md">
                  {market.summary}
                </p>
        ...
        <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-8 lg:grid-cols-12 lg:items-stretch">
          <FadeIn className="lg:col-span-5">
            <div className="relative h-full min-h-[420px] overflow-hidden border border-white/10">
              <ResponsiveImage
                ...
                alt={`${market.title} project showcase`}
                width={1600}
                height={1200}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-7">
                ...
                <h2 className="mt-3 text-4xl font-black leading-tight text-white">
                  Documented project stages from start to finish.
                </h2>
              </div>
            </div>
          </FadeIn>
          <div className="grid gap-4 lg:col-span-7 md:grid-cols-3">
            {[
              {
                icon: Camera,
                title: 'Remote Visual Scope',
                body: 'Upload or link photos of your rooms or surfaces directly in our estimate form for a faster, more precise preliminary review.',
              },
              {
                icon: ClipboardCheck,
                title: 'Streamlined Intake',
                body: 'Our simple multi-step form gathers your project details so we arrive fully prepared.',
              },
              {
                icon: FileCheck2,
                title: 'Itemized Proposals',
                body: 'We provide highly detailed, transparent scopes specifying exact preparation, paint products, and linear measurements.',
              },
            ].map((card, index) => (
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 lg:grid-cols-12">
          <FadeIn className="lg:col-span-4">
            ...
            <h2 className="mt-4 text-4xl font-black leading-tight text-white">
              Engineered to meet the exact standards of your project.
            </h2>
          </FadeIn>
            ...
            {market.capabilities.map((capability, index) => (
              <FadeIn key={capability.title} delay={0.08 * index}>
                <article className="market-panel h-full border border-white/10 bg-black-charcoal p-6">
                  <span className="mb-8 block font-display text-4xl font-black text-white/80">
                    0{index + 1}
                  </span>
          ...
          <FadeIn>
            <div className="max-w-3xl">
              <p className="text-sm font-medium text-white">
                Scope / Prep / Execute / Verify
              </p>
              <h2 className="mt-4 text-4xl font-black leading-tight text-white md:text-5xl">
                A disciplined process for better outcomes.
              </h2>
            </div>
          </FadeIn>
            ...
            {market.process.map((step, index) => (
              <FadeIn key={step.title} delay={0.08 * index}>
                <article className="h-full bg-black-primary p-6">
                  <p className="mb-8 text-sm font-medium text-white">
                    Step 0{index + 1}
                  </p>
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 lg:grid-cols-12 lg:items-center">
          <FadeIn className="lg:col-span-7">
            <h2 className="max-w-3xl text-4xl font-black leading-tight text-white md:text-5xl">
              Insured, owner-led, and built for real scope clarity. (MN ID:
              IR816596 | MN Statute 176.041 Worker's Comp Exemption for
              owner-operators)
            </h2>
          ...
          <FadeIn delay={0.1} direction="left" className="lg:col-span-5">
            <div className="border border-white/30 bg-white/10 p-6">
              ...
              <Link
                ...
                className="mt-8 inline-flex w-full items-center justify-center gap-2 border border-white/35 bg-white/10 px-6 py-4 text-sm font-black text-white transition-colors hover:border-white hover:text-white"
              >
              ...
              <Link
                ...
                className="mt-3 inline-flex w-full items-center justify-center gap-2 bg-white px-6 py-4 text-sm font-black text-[#050505] transition-colors hover:bg-gray-200"
              >
                {market.cta} <ArrowRight size={18} />
              </Link>
              <BookingCta
                audience={
                  market.slug === 'residential' ? 'homeowner' : market.slug
                }
