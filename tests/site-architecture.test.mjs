...
test('top-level routes use the three-market architecture in Next.js filesystem routing', () => {
  ...
  assert.ok(
    existsSync(
      new URL('../src/app/service-areas/[slug]/page.tsx', import.meta.url)
    )
  );
  assert.ok(
    existsSync(
      new URL('../src/app/painting-services/[slug]/page.tsx', import.meta.url)
    )
  );
});
...
test('primary navigation leads with residential, commercial, and public sector', () => {
  ...
  assert.ok(
    publicSector > commercial,
    'Public Sector should follow Commercial'
  );
...
test('homepage states the approved positioning and avoids forbidden claims', () => {
  ...
  assert.doesNotMatch(
    home,
    /Licensed|Bonded|MnDOT-approved|Government-certified|DBE certified|TGB certified|Trusted by government agencies|Awarded public contracts|Workers comp/i
  );
});
