...
1. Confirm no package install is running in the shared worktree.
...
2. Run `scripts/restore-node-modules-local.ps1 -RepositoryPath <absolute-path> -ExpectedTargetPath <absolute-E-target>`. It copies into a staging directory on the repository drive, checks file count and byte count, removes only the verified junction, and promotes the staging directory. The E: dependency tree remains untouched.
