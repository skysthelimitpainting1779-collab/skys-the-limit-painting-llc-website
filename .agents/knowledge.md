...
- **Linting**: Run TypeScript checks via `powershell -ExecutionPolicy Bypass -Command "npm run lint"`
- **Testing**: Run test suite via `powershell -ExecutionPolicy Bypass -Command "npm test"`
- **Next.js Production Compilation**: Run build via `powershell -ExecutionPolicy Bypass -Command "npm run build"`
- **Project Graph Compilation**: Compile master graph via `powershell -ExecutionPolicy Bypass -File "..\compile-all.ps1"`
